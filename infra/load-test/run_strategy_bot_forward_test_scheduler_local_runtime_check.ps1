...
$reportPath = Join-Path $ReportsDir "strategy-bot-forward-test-local-runtime-check-$timestamp.md"
$appLogPath = Join-Path $ReportsDir "strategy-bot-forward-test-local-runtime-app-$timestamp.log"
$appErrLogPath = Join-Path $ReportsDir "strategy-bot-forward-test-local-runtime-app-$timestamp.err.log"
...
try {
  ...
  $beforeChild = Get-LatestReport -Directory $ReportsDir -Pattern "strategy-bot-forward-test-scheduler-smoke-*.md"
  $beforeTimestamp = if ($null -ne $beforeChild) { $beforeChild.LastWriteTimeUtc } else { [datetime]::MinValue }

  & (Join-Path $PSScriptRoot "run_strategy_bot_forward_test_scheduler_smoke.ps1") `
    -BaseUrl $baseUrl `
  ...
  $afterChild = Get-LatestReport -Directory $ReportsDir -Pattern "strategy-bot-forward-test-scheduler-smoke-*.md"
...
$lines = @()
$lines += "# Strategy Bot Forward-Test Local Runtime Check"
$lines += ""
...
Write-Host "Strategy bot forward-test local runtime report created: $reportPath"
