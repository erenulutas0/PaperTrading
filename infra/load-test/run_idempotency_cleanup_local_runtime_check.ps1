...
$reportPath = Join-Path $ReportsDir "idempotency-cleanup-local-runtime-check-$timestamp.md"
$appLogPath = Join-Path $ReportsDir "idempotency-cleanup-local-runtime-app-$timestamp.log"
$appErrLogPath = Join-Path $ReportsDir "idempotency-cleanup-local-runtime-app-$timestamp.err.log"
...
try {
  ...
  $beforeChild = Get-LatestReport -Directory $ReportsDir -Pattern "idempotency-cleanup-smoke-*.md"
  $beforeTimestamp = if ($null -ne $beforeChild) { $beforeChild.LastWriteTimeUtc } else { [datetime]::MinValue }

  & (Join-Path $PSScriptRoot "run_idempotency_cleanup_smoke.ps1") `
    -BaseUrl $baseUrl `
    -OutputDir $ReportsDir `
    -TimeoutSec $TimeoutSec `
    -NoFail

  $afterChild = Get-LatestReport -Directory $ReportsDir -Pattern "idempotency-cleanup-smoke-*.md"
...
$lines = @()
$lines += "# Idempotency Cleanup Local Runtime Check"
$lines += ""
...
Write-Host "Idempotency cleanup local runtime report created: $reportPath"
