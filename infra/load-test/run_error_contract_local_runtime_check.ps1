...
$reportPath = Join-Path $ReportsDir "error-contract-local-runtime-check-$timestamp.md"
$appLogPath = Join-Path $ReportsDir "error-contract-local-runtime-app-$timestamp.log"
$appErrLogPath = Join-Path $ReportsDir "error-contract-local-runtime-app-$timestamp.err.log"
...
try {
  ...
  $beforeChild = Get-LatestReport -Directory $ReportsDir -Pattern "error-contract-staging-checklist-*.md"
  $beforeTimestamp = if ($null -ne $beforeChild) { $beforeChild.LastWriteTimeUtc } else { [datetime]::MinValue }

  & (Join-Path $PSScriptRoot "run_error_contract_staging_checklist.ps1") `
    -BaseUrl $baseUrl `
    -ReportsDir $ReportsDir `
    -TimeoutSec $TimeoutSec `
    -NoFail

  $afterChild = Get-LatestReport -Directory $ReportsDir -Pattern "error-contract-staging-checklist-*.md"
...
$lines = @()
$lines += "# Error Contract Local Runtime Check"
$lines += ""
...
Write-Host "Error contract local runtime report created: $reportPath"
