...
$reportPath = Join-Path $ReportsDir "notification-sse-fallback-local-runtime-check-$timestamp.md"
$appLogPath = Join-Path $ReportsDir "notification-sse-fallback-local-runtime-app-$timestamp.log"
$appErrLogPath = Join-Path $ReportsDir "notification-sse-fallback-local-runtime-app-$timestamp.err.log"
...
try {
  ...
  $beforeChild = Get-LatestReport -Directory $ReportsDir -Pattern "notification-sse-fallback-staging-checklist-*.md"
  $beforeTimestamp = if ($null -ne $beforeChild) { $beforeChild.LastWriteTimeUtc } else { [datetime]::MinValue }

  & (Join-Path $PSScriptRoot "run_notification_sse_fallback_staging_checklist.ps1") `
    -BaseUrl $baseUrl `
    -ReportsDir $ReportsDir `
    -TimeoutSec $TimeoutSec `
    -DuplicateWindowSec $DuplicateWindowSec `
    -NoFail

  $afterChild = Get-LatestReport -Directory $ReportsDir -Pattern "notification-sse-fallback-staging-checklist-*.md"
...
$lines = @()
$lines += "# Notification SSE Fallback Local Runtime Check"
$lines += ""
...
Write-Host "Notification SSE fallback local runtime report created: $reportPath"
