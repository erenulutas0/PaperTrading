...
$reportPath = Join-Path $ReportsDir "auth-strict-post-cutover-local-runtime-check-$timestamp.md"
$appLogPath = Join-Path $ReportsDir "auth-strict-post-cutover-local-runtime-app-$timestamp.log"
$appErrLogPath = Join-Path $ReportsDir "auth-strict-post-cutover-local-runtime-app-$timestamp.err.log"
...
try {
  if (-not $SkipAppStart) {
    ...
    $appCommand = @(
      ...
      "`$env:APP_AUTH_ALLOW_LEGACY_USER_ID_HEADER='false'",
  ...
  $beforeChild = Get-LatestReport -Directory $ReportsDir -Pattern "auth-strict-post-cutover-checklist-*.md"
  ...
  if ($SkipBaseline) { $childParams["SkipBaseline"] = $true }
  if ($SkipRelay) { $childParams["SkipRelay"] = $true }
  if ($SkipSpoof) { $childParams["SkipSpoof"] = $true }
  if ($SkipRateLimit) { $childParams["SkipRateLimit"] = $true }

  & (Join-Path $PSScriptRoot "run_auth_strict_post_cutover_checklist.ps1") @childParams | Out-Null

  $afterChild = Get-LatestReport -Directory $ReportsDir -Pattern "auth-strict-post-cutover-checklist-*.md"
...
$lines = @()
$lines += "# Auth Strict Post-Cutover Local Runtime Check"
$lines += ""
...
$lines += "- Legacy Header Acceptance Forced Off: true"
...
Write-Host "Auth strict post-cutover local runtime report created: $reportPath"
