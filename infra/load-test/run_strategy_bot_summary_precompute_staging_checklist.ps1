...
if ([string]::IsNullOrWhiteSpace($BaseUrl)) {
  throw "BaseUrl is required for the strategy-bot summary precompute staging checklist."
}
...
$reportPath = Join-Path $ReportsDir "strategy-bot-summary-precompute-staging-checklist-$timestamp.md"
$smokeScript = Join-Path $scriptDir "run_strategy_bot_summary_precompute_smoke.ps1"
$healthUrl = "$BaseUrl/actuator/health"
$results = New-Object 'System.Collections.Generic.List[object]'
$notes = New-Object 'System.Collections.Generic.List[string]'
$restartLogPath = Join-Path $ReportsDir "strategy-bot-summary-precompute-staging-restart-$timestamp.log"
...
$beforeSmoke = Get-LatestReportSnapshot -Directory $ReportsDir -Pattern "strategy-bot-summary-precompute-smoke-*.md"
& $smokeScript `
  -BaseUrl $BaseUrl `
  ...
  -NoFail
$afterSmoke = Get-LatestReportSnapshot -Directory $ReportsDir -Pattern "strategy-bot-summary-precompute-smoke-*.md"
...
if ([string]::IsNullOrWhiteSpace($smokeReport)) {
  $results.Add((New-StepResult -Name "Summary Precompute Smoke" -Status "FAILED" -Detail "report_not_found")) | Out-Null
} else {
  $smokeStatus = Get-StatusFromReport -ReportPath $smokeReport
  if ([string]::IsNullOrWhiteSpace($smokeStatus)) {
    $smokeStatus = "FAILED"
  }
  $results.Add((New-StepResult -Name "Summary Precompute Smoke" -Status $smokeStatus -Detail "report=$smokeReport" -ReportPath $smokeReport)) | Out-Null
}
...
$lines = @()
$lines += "# Strategy Bot Summary Precompute Staging Checklist"
$lines += ""
...
$lines += ""
$lines += "Coverage:"
$lines += "- verify target runtime is healthy before summary precompute validation"
$lines += "- optionally run a caller-supplied restart command before smoke validation"
$lines += "- run the actuator-backed strategy-bot summary precompute smoke with staging-friendly polling"
$lines += ""
...
Write-Host "Strategy bot summary precompute staging checklist report created: $reportPath"
