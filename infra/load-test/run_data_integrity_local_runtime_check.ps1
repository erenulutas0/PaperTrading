...
$reportPath = Join-Path $ReportsDir "data-integrity-local-runtime-check-$timestamp.md"
$results = New-Object 'System.Collections.Generic.List[object]'

$results.Add((Invoke-ScriptStep `
      -Name "Market Price False-Loss Local Runtime Check" `
      -ScriptPath (Join-Path $scriptDir "run_market_price_false_loss_local_runtime_check.ps1") `
      -Parameters @{
        ReportsDir = $ReportsDir
        NoFail     = $true
      } `
      -ReportPattern "market-price-false-loss-local-runtime-check-*.md")) | Out-Null
...
$lines = @()
$lines += "# Data Integrity Local Runtime Check"
$lines += ""
$lines += "- Timestamp: $(Get-Date -Format s)"
$lines += "- Status: **$overallStatus**"
$lines += ""
$lines += "Coverage:"
$lines += "- fresh local market-price false-loss runtime validation"
$lines += "- local Flyway/data integrity validation for V3/V9/V10/V27 plus zeroed crypto quantity inspection"
$lines += ""
...
Write-Host "Data integrity local runtime check report created: $reportPath"
